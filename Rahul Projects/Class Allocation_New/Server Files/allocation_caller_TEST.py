...
async def dataQualifying(tableName: Optional[str] = Form(...), colsArray: Optional[str] = Form(...), accessName: Optional[str] = Form(...), SPIR_NO: Optional[str] = Form(None), analysisType: Optional[str] = Form(...), responseId: Optional[str] = Form(...), createBy: Optional[str] = Form(None), BATCH_ID: Optional[str] = Form(None)):
    ...
    if analysisType == 'CLASS_ALLOCATION':
        ...
        file = pd.ExcelFile(r"C:\Users\Administrator\PythonDeployment\Class Allocation_New\Server Files\CHAR_EXCELS\ISML UOM_1.xls")
        ...
        # prop_cndtn_df = pd.read_excel(r"C:\Users\Administrator\PythonDeployment\Class Allocation_New\Server Files\CHAR_EXCELS\Property_Rules.xlsx")  #WORKING FINE 05.09.22
        prop_cndtn_df = pd.read_excel(r"C:\Users\Administrator\PythonDeployment\Class Allocation_New\Server Files\CHAR_EXCELS\Property_Rules_05.09.22.xlsx", "ORPIC")  # UPDATED
        # prop_cndtn_df = pd.read_excel(r"C:\Users\Administrator\PythonDeployment\Class Allocation_New\Server Files\CHAR_EXCELS\Property_Rules1.xlsx")
        ...
        if BATCH_ID:
            ...
            print('---->', b_split_str.strip(","))
            b_split_str = b_split_str.strip(",")
            s_qry = """ select {} from {} where BATCH_ID in ({}) and PART_REF_ISSUED IS NOT NULL """.format(colsArray, tableName, b_split_str)
        elif SPIR_NO:
            b_split_str = "".join("'" + i + "'," for i in SPIR_NO.split(','))
            print('---->', b_split_str.strip(","))
            b_split_str = b_split_str.strip(",")
            s_qry = """ select {} from {} where SPIR_NO in ({}) and PART_REF_ISSUED IS NOT NULL """.format(colsArray, tableName, b_split_str)
        ...
        #                                                                      desc_cls_file)
        charallocation_dataset = CHAR_ALLOCATION_LIVE.char_allocation(clause_df, prop_cndtn_df, char_data, 
                                                                        desc_cls_file,uom_replace_df,UOM,UOM1,df)
        ...
        # i_qry = """ insert into TEMP_PYTHON_PANDAS(RESPONSE_ID, RES_DATA) values(:RESPONSE_ID, :RES_DATA )"""
        # i_qry = """ insert into PYTHON_API_RESPONSE(RESPONSE_ID, RESPONSE_CATEGORY, RESPONSE_DATA, INSERT_DATE) values(:RESPONSE_ID, :RESPONSE_CATEGORY, :RESPONSE_DATA, :INSERT_DATE)"""  #previous one
        i_qry = """ insert into O_RECORD_WS_RESP_PYTHON_API(RESPONSE_ID, RESPONSE_CATEGORY, RESPONSE_DATA, CREATE_DATE) values(:RESPONSE_ID, :RESPONSE_CATEGORY, :RESPONSE_DATA, :CREATE_DATE)"""
        ...
        res_cr.execute(i_qry, RESPONSE_ID=responseId, RESPONSE_CATEGORY='Char Extraction',
                       RESPONSE_DATA=str(return_response_data), CREATE_DATE=(datetime.now()))
        res_conn.commit()
        # =============================================================================

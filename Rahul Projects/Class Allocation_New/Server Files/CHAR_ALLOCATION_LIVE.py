...
def char_allocation(clause_data,property_cndtn_data,class_data, desc_cls_file, uom_replace_df,UOM,UOM1,df):
...
#-------------------------------------------------------------------------------------------------------------------
    ...
    # classData.to_excel('classData11.xlsx', index = False)
    ...
    classData_new.reset_index(drop= True, inplace = True)
    classData_new.to_excel('classData_new_Error.xlsx', index = False)
    ...
    # =======================================================================================================================
        ...
        # classData_new1['uom'] = classData_new1.apply(lambda x: uom_val(x['Prop_Val']) if ((x['uom_status'] == 1) & (x['uom'] == '')) else x['uom'], axis=1)
    ...
    # if len(classData_new1) != 0:
        ...
        classData_new1['Prop_Val'] = classData_new1.apply(lambda x: re.sub(x['Prop_Name'], '', x['Prop_Val']),axis=1)

        # classData_new1['Prop_Val'] = classData_new1.apply(lambda x: re.sub(x['Prop_Val'], '', x['Prop_Val']), axis = 1)

        classData_new1['Prop_Val'] = classData_new1.apply(lambda x: re.sub(x['uom'], '', x['Prop_Val']), axis = 1)
    ...
    #     ==============================================================================================================
        ...
        final_df2.drop(['Properties', 'Quantity', 'r_uom', 'des1', 'uom_status'], axis=1, inplace=True)

    else:
        final_df2 = pd.DataFrame()
        classData['LONG_DESCRIPTION_EXCLUDE'] = ''
        print('rrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrr \n', classData)
        group3 = classData.groupby(['MDRM'])
        for i in (classData['MDRM'].unique()):
            batch3 = group3.get_group(i)
            batch3.reset_index(drop=True, inplace=True)
            batch3['LONG_DESCRIPTION_EXCLUDE'] = batch3.apply(lambda x: long_des_exclude(batch3), axis=1)
            final_df2 = pd.concat([final_df2, batch3], ignore_index=True)
...
# print(val)
    ...
    final_df2.to_excel('output_char_extraction_spir_exclude_data_06.09.22.xlsx', index=False)

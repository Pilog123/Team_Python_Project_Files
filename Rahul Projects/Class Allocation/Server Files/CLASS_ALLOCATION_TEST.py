...
def class_allocation_func(source_DF):
    ...
    def altr_word(des):
        des = des.upper()
        des = re.sub('ASSEMBLY:','ASSEMBLY',des)
        des = re.sub('(;|:)',',',des)
    ...
    def obj_qual_cndtn(x): 
        objct = []
        x_list = [i[:1] for i in list(set([re.sub('\W','','{}'.format(i)) for i in x.split()]))]
        x_lst = [i for i in x_list if i.isalpha()]
        # print(x_lst, x)
        try:
            cl_ck_df = pd.concat([dict_df['{}'.format(i)] for i in x_lst if len(x_lst) != 0])
            for obj in cl_ck_df:
    ...
    def final_class(des2,r1,r1_cls,ob,obj_cls,qual):
        des2 = re.sub(r'\([^()]*\)', ' ', des2)
        des2 = re.sub(r' +',' ',des2)
        # des1 = des
    ...
    try:
        ...
        source_DF['RULE1'] = source_DF['New_Source_Long_Description'].progress_apply(lambda des:rule1(des))
    except:
    ...
    try:
        source_DF['CLASS2'] = source_DF[source_DF['Object'].notnull()].apply(lambda x: fnl_cls(x.Object, 
    ...
    source_DF['FINAL_CLASS'] = source_DF.apply(lambda x: final_class(x.New_Source_Long_Description, x.RULE1_STRING,
                                                            x.CLASS_TYPE1, x.Object_, x.CLASS2,x.Obj_Qualifier),axis=1)
    ...
    source_DF['CLASS_TYPE0'] = source_DF[source_DF['FINAL_CLASS'].isnull()]['Alter_Source_Long_Description'].apply(lambda des:frst_chk(des))
    ...
    source_DF['Object_2'] = source_DF[source_DF['CLASS_TYPE0'].isnull() & source_DF['FINAL_CLASS'].isnull()
                    ]['New_Source_Long_Description'].apply(lambda des:object2(des))
    ...
    try:
        source_DF['CLASS21'] = source_DF[source_DF['Object2_']!=''].apply(lambda x: fnl_cls(x.Object2_, x.Obj_Qualifier2), axis=1)
    ...
    source_DF['CLASS_LAST'] = source_DF[source_DF['FINAL_CLASS'].isnull()  & source_DF['CLASS21'].isnull() &
                    source_DF['CLASS_TYPE0'].isnull()]['Alter_Source_Long_Description'].apply(lambda des:last_cls(des))
    ...
    source_DF['NEW_CLASS'] = source_DF[['CLASS_TYPE0','FINAL_CLASS','CLASS_LAST',
                    'CLASS21']].apply(lambda x : '{}{}{}{}'.format(x[0],x[1],x[2],x[3]), axis=1)
    ...
    source_DF.drop(['Alter_Source_Long_Description', 'New_Source_Long_Description', 'RULE1',

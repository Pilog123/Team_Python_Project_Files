...
async def dataQualifying(tableName: Optional[str] = Form(...), colsArray: Optional[str] = Form(...), accessName: Optional[str] = Form(...), BATCH_ID: Optional[str] = Form(...), analysisType: Optional[str] = Form(...)):
    ...
    if analysisType == 'UNSPSC_ALLOCATION':
    ...
    elif analysisType == 'CLASS_ALLOCATION':
        s_qry = """ select {} from {} where BATCH_ID = '{}' """.format(colsArray, tableName, BATCH_ID)
        print(s_qry)
        data = CallingDatabaseTable(accessName, s_qry)
        print(type(data))
        # classAllication_dataset = ONLY_CLASS_TEST.class_allocation_func(data)
        classAllication_dataset = CLASS_ALLOCATION_TEST.class_allocation_func(data[:5000])
        ...
        prop_cndtn_df = pd.read_excel("Is_properties_condition_working_23.12.21.xls").iloc[:, :9]
        s_qry = """ select {} from {} where BATCH_ID = '{}' """.format(colsArray, tableName, BATCH_ID)
        print(s_qry)
        ...
        charallocation_dataset = CHAR_ALLOCATION_CHANGED.char_allocation(clause_df, prop_cndtn_df, char_data[:100], UOM, UOM1)
...
if __name__ == '__main__':
    uvicorn.run("allocation_caller_TEST:allocationApp", host='172.16.1.60', port=6648, log_level='info')

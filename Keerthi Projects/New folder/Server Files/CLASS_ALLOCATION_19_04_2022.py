...
def class_allocation_func(source_DF):
    ...
    # CLASS TABLE
    check_df = pd.ExcelFile(r'C:\Users\Administrator\PythonDeployment\Class Allocation\Server Files\CLASS_EXCELS\CLASS_CLEAN_Item_Details.xlsx')
    # check_df = pd.ExcelFile(r'./Item Details/CLASS_CLEAN_Item_Details.xlsx')
    ...
    alter_df = pd.read_excel(r'C:\Users\Administrator\PythonDeployment\Class Allocation\Server Files\CLASS_EXCELS\IS_ALTER_WORD_ABB.xlsx')
    ...
    ### Exclusions ###
    exmp = pd.ExcelFile(r'C:\Users\Administrator\PythonDeployment\Class Allocation\Server Files\CLASS_EXCELS\Exclusions.xlsx')
                ...
                if ',' in cls:
                    if (re.search(r'\b{}\b'.format(str(ob)),re.sub(r'(,).*','',cls))) and (re.search(r'\b{}\b'.format(re.sub(r'.*(,)(\s)','',cls)),qu)):
                        if ', '.join([re.search(ob,re.sub(r'(,).*','',cls)).group(), re.search(r'\b{}\b'.format(re.sub(r'.*(,)(\s)','',cls)),qu).group()]) == cls:
                            ...
                            if '|' in ob:
                                return [re.sub(r'(\|).*','',r'{}\b'.format(ob)),des1]
                            else:
                                return [ob, des1]
                else:
    ...
    def last_cls(des):
        ...
        for lst_cls, lst_obj in zip(lst_check['Class'],lst_check['TERM(OBJECT)']):
            ...
            if mat:
                ...
                if re.search(r'\b({})\b'.format(lst_obj),des1):
    ...
    def final_class(des,des2,r1,r1_cls,ob,obj_cls,qual):
        ...
        if (r1_cls != '') and (obj_cls != ''):
            # print('.....................................')
        # if ((r1_cls != '') and (obj_cls != '')):
            print('type:---',type(r1_cls),r1_cls,'type:---',type(obj_cls),obj_cls)
            ob = re.sub(r'\^','',ob)
            if re.search(r'({})'.format(str(obj_cls)),str(r1_cls)):
    ...
    source_DF['First_CLASS_'] = source_DF['New_Source_Long_Description'].apply(lambda des:multi_cls(des))
    ...
    source_DF.replace({'CLASS_TYPE1':{np.NaN:''},'RULE1_STRING':{np.NaN:''}, 'Object_':{np.NaN:''},
                                                                    'CLASS2':{np.NaN:''}},inplace=True)
    # source_DF.replace('',np.NaN,inplace=True)
    # source_DF['CLASS_TYPE1'].replace(np.NaN, '', inplace=True)
    # source_DF['RULE1_STRING'].replace(np.NaN, '', inplace=True)
    # source_DF['Object_'].replace(np.NaN, '', inplace=True)
    # source_DF['CLASS2'].replace(np.NaN, '', inplace=True)

    # source_DF.to_csv('test.csv')
    source_DF['FINAL_CLASS_'] = source_DF[source_DF['First_CLASS'].isnull()].apply(lambda x: final_class(x.Alter_Source_Long_Description,
    ...
    source_DF['CLASS_TYPE0_'] = source_DF[source_DF['FINAL_CLASS'].isnull() & 
                        source_DF['First_CLASS'].isnull()]['Alter_Source_Long_Description'].apply(lambda des:frst_chk(des))

    source_DF['CLASS_LAST_'] = source_DF[source_DF['FINAL_CLASS'].isnull() & source_DF['First_CLASS'].isnull() &
                        source_DF['CLASS_TYPE0_'].isnull()]['Alter_Source_Long_Description'].apply(lambda des:last_cls(des))
    ...
    source_DF['Object_2'] = source_DF[source_DF['CLASS_TYPE0'].isnull() & source_DF['FINAL_CLASS'].isnull() & source_DF['First_CLASS'].isnull() &
                        source_DF['CLASS_LAST'].isnull()]['New_Source_Long_Description'].apply(lambda des:object2(des))
    ...
    try:
        source_DF['CLASS2_'] = source_DF[source_DF['Object2_']!=''].apply(lambda x: fnl_cls(x.Alter_Source_Long_Description,
                                                                                    x.Object2_, x.Obj_Qualifier2), axis=1)
        
    except:
        source_DF['CLASS2_'] = np.NaN
    ...
    source_DF.replace('No Class','',inplace=True)
    ...
    source_DF.drop(['Alter_Source_Long_Description', 'New_Source_Long_Description',
        'First_CLASS_', 'First_CLASS', 'DESCRIPTION_FFT0', 'RULE1',
        'CLASS_TYPE1', 'RULE1_STRING', 'Object_', 'Object1', 'Object2',
        'Object', 'Obj_Qualifier', 'CLASS2', 'FINAL_CLASS_', 'FINAL_CLASS',
        'DESCRIPTION_FFT1', 'CLASS_TYPE0_', 'CLASS_LAST_', 'CLASS_TYPE0',
        'DESCRIPTION_FFT2', 'CLASS_LAST', 'DESCRIPTION_FFT3', 'Object_2',
        'Object21', 'Object22', 'Object2_', 'Obj_Qualifier2', 'CLASS2_',
        'CLASS21', 'DESCRIPTION_FFT4'
            ],axis=1,inplace=True)
...
#########################################################################################################################################################################################################################################
def unspsc_allocation_func(source_dataframe, insert_conn_str):
    ...
    conn = cx_Oracle.connect(insert_conn_str)
    print('Connected  at UNSPSC!')
    ...
    print(source_dataframe['UNSPSC_CODE_DATA'])
...
# source_DF = pd.read_excel(r'MEDICAL_DATA_RES.xlsx')

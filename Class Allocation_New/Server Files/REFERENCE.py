...
def reference_data(sample_df,part_prefix_df,isml_uom):
    ...
    sample_df = sample_df.rename(columns={'ITEM_DESC': 'LONG_TEXT'})
    sample_df.rename(columns={'SPIR_NO':'MATERIAL','LONG_TEXT':'LONG_DESCRIPTION'},inplace=True)
    ...
    for i in (range(df_combined.shape[0])):
        if df_combined['Reference_Type'][i] in vendor_list:
            df_combined['VENDOR'][i] = df_combined['Reference_Number'][i]
    ...
    #----------------------------------------------------
    df_combined.to_excel('ref_excel.xlsx')

    # df_combined = df_combined[['RECORD_NO', 'PROPERTY_NAME_DATA', 'PROPERTY_VALUE_DATA', 'UOM_DATA']]

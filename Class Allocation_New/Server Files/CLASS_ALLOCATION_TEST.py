...
def class_allocation_func(source_DF):
    # source_DF = source_DF.rename(columns={'ITEM_DESC': 'LONG_TEXT'})
    source_DF = source_DF.rename(columns={'LONG_DESCRIPTION': 'LONG_TEXT'})
...
def unspsc_allocation_func(source_dataframe):
    ...
    # qry = """"SELECT TERM,UNSPSC_CODE,UNSPSC_DESC,UNSPSC_CODE_SEGMENT_LEVEL,UNSPSC_DESC_SEGMENT_LEVEL,UNSPSC_CODE_FAMILY_LEVEL,UNSPSC_DESC_FAMILY_LEVEL FROM ONTG_CLASSIFICATION""""
    unspsc = pd.read_sql(
    ...
    print(source_dataframe.columns)
    source_dataframe =  source_dataframe[['MATERIAL_NO', 'SHORT_DESCRIPTION', 'CLASS', 'UNSPSC_CODE',
     'UNSPSC_DESCRIPTION', 'UNSPSC_SEGMENT', 'UNSPSC_FAMILY', 'UNSPSC_CLASS',
     'BATCH_ID', 'CREATE_BY', 'EDIT_BY',
     'AUDIT_ID', 'LONG_TEXT', 'NEW_CLASS', 'UNSPSC_CODE_DATA',
     'UNSPSC_DESCRIPTION_DATA', 'UNSPSC_SEGMENT_DATA', 'UNSPSC_FAMILY_DATA',
     'UNSPSC_CLASS_DATA', 'ISIC_CLASS_DATA']]
...
#         if analysisType == 'CLASS Allocation':
...
#     uvicorn.run("CLASS_Allocation 25-01:app", port= 5555, reload=True, access_log=False)